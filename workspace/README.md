...
PRD mechanical validation uses `kits/prd-creator/validator/validate_prd.py`. PRD → Voice handoff uses `kits/prd-creator/validator/validate_handoff.py` only at that boundary. Voice mechanical validation uses `kits/prd-creator/validator/validate_voice.py` only when Voice scope exists.
...
`approved_preview` is an intentional pre-handoff project state, not an incomplete `handoff_ready` package.

```text
approved_preview
→ approved project-specific preview may remain under state/ + work/
→ formal output/ versioned bundle is not required yet
→ does not authorize Flow 5 handoff by itself
→ advance only when the user requests formalization or another downstream boundary
```

When this exception is used, current project state must identify the retained approved preview. The preview is removed after it is superseded by a formal versioned handoff or another explicitly approved replacement.
