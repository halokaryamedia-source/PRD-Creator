...
class ProjectHtmlVoiceAssets(unittest.TestCase):
    ...
    def test_voice_production_is_composed_into_same_prd_html(self) -> None:
        ...
        # Production Assets extends the original PRD navigation.
        self.assertIn('class="nav-submenu phase-navigation"', html)
        self.assertIn('data-section-code="04"', html)
        self.assertNotIn("production-objective-nav", html)
        self.assertIn('data-en="04A" data-id="04A">04A</span>', html)
        self.assertNotIn('data-en="05A" data-id="05A">05A</span>', html)

        # One Voice category shows section name + accepted gameplay label.
        ...
        self.assertEqual(html.count('class="production-assets-category"'), 1)
        self.assertEqual(html.count('data-target="production-assets-voice-'), 2)
        ...
        # Voice page identity no longer borrows synthetic PRD navigation numbers.
        self.assertIn('class="voice-objective-title">Core Trial</h2>', html)
        self.assertNotIn('class="voice-objective-title">05. Core Trial</h2>', html)
    ...
    def test_prd_without_voice_keeps_production_assets_absent(self) -> None:
        rendered, output = self.render(self.make_project(include_voice=False))
        self.assertEqual(rendered.returncode, 0, rendered.stderr or rendered.stdout)
        html = output.read_text(encoding="utf-8")
